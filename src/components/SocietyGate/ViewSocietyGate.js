...
export const ViewSocietyGate = (props) => {
...
const SocietyDetail = (props) => {
  ...
  const handleSearch = (e) => {
    let arr = props.payload.society_gates.filter(
      (item) =>
        item.name.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.description.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.status.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.address_1.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.address_2.toLowerCase().includes(e.target.value.toLowerCase())
    );
    console.log(arr);
    if (arr.length > 0) {
      setData(arr);
  ...
  const handleView = (item) => {
    item.title = "View " + item.name + " Society Gate Detail";
    item.backText = "View Society Gate";

    props.history.push({ pathname: "viewPopUp/", state: item });
  };

  const handleEdit = (item) => {
    props.history.push({ pathname: "/addSocietyGate", state: item });
  };
  ...
  return (
    <div className="row">
      <div className="col-12 px-4 pt-4 pb-3">
        <div className={styles.title}>Society Gate Details </div>
      </div>
          ...
          <div className="col-12 px-4 pb-4 col-md-5">
            <div className={`${styles.formInput} form-group mb-3`}>
              <input
                ...
                placeholder="Search society gate"
