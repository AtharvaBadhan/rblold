...
import { getAllResidentMemberApi } from "../../../util/service";
...
const SocietyDetail = (props) => {
  ...
  useEffect(() => {
    if (props.payload) {
      setData(props.payload.society_members);
    }
  }, [props.payload]);

  const handleSearch = (e) => {
    let arr = props.payload.society_members.filter(
      (item) =>
        item.name.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.mobile_1.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.email_1.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.address_1.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.state.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.city.toLowerCase().includes(e.target.value.toLowerCase()) ||
        item.pincode.toLowerCase().includes(e.target.value.toLowerCase()) ||
  ...
  const handleView = (item) => {
    item.title = "View " + item.name + " Resident Detail";
    item.backText = "View Resident Member";
    props.history.push({ pathname: "viewPopUp/", state: item });
  };

  const handleEdit = (item) => {
    props.history.push({ pathname: "/addResident", state: item });
  };
  ...
  return (
    <div className="row">
      <div className="col-12 px-4 pt-4 pb-3">
        <div className={styles.title}>Resident Details </div>
      </div>
          ...
          <div className="col-12 px-4 pb-4 col-md-5">
            <div className={`${styles.formInput} form-group mb-3`}>
              <input
                ...
                placeholder="Search resident member"
